...
class ExamCentre(Base, UUIDPrimaryKeyMixin, TimestampMixin, SoftDeleteMixin):
    ...
    centre_allocations: Mapped[list[CentreAllocation]] = relationship(
        "CentreAllocation",
        back_populates="centre",
        cascade="all, delete-orphan",
    )

    admit_cards: Mapped[list[AdmitCard]] = relationship(
        "AdmitCard",
        back_populates="centre",
    )
...
class CentreAllocation(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    centre_id: Mapped[PyUUID] = mapped_column(
        SA_UUID(as_uuid=True),
        ForeignKey("exam_centres.id"),
        nullable=False,
    )

    allocated_by_user_id: Mapped[PyUUID | None] = mapped_column(
        SA_UUID(as_uuid=True),
        ForeignKey("users.id"),
        nullable=True,
    )

    allocation_reason: Mapped[str | None] = mapped_column(
        Text,
        nullable=True,
    )

    ai_recommended: Mapped[bool] = mapped_column(
        Boolean,
        nullable=False,
        default=False,
    ...
    application: Mapped[Application] = relationship(
        "Application",
        back_populates="centre_allocation",
    )

    centre: Mapped[ExamCentre] = relationship(
        "ExamCentre",
        back_populates="centre_allocations",
    )
...
class AdmitCard(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    reporting_time: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True),
        nullable=True,
    )

    exam_time: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True),
        nullable=True,
    )

    centre_id: Mapped[PyUUID | None] = mapped_column(
        SA_UUID(as_uuid=True),
        ForeignKey("exam_centres.id"),
        nullable=True,
    )

    pdf_document_id: Mapped[PyUUID | None] = mapped_column(
        SA_UUID(as_uuid=True),
        ForeignKey("documents.id"),
        nullable=True,
    )

    qr_code_value: Mapped[str | None] = mapped_column(
        String(255),
        nullable=True,
        unique=True,
    )
    ...
    generated_by_user_id: Mapped[PyUUID | None] = mapped_column(
        SA_UUID(as_uuid=True),
        ForeignKey("users.id"),
        nullable=True,
    )
    ...
    application: Mapped[Application] = relationship(
        "Application",
        back_populates="admit_card",
    )

    centre: Mapped[ExamCentre | None] = relationship(
        "ExamCentre",
        back_populates="admit_cards",
    )
